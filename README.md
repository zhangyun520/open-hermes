...
Cognitive Jelly 的版本不是 `v0.1 → v0.2 → v0.3` 的单线升级，而是 DAG / 图结构上的多声部并行演化。一个对象可以同时存在 Stable 主旋律、Experimental 实验声部、Safety 安全声部、Aesthetic 美学声部、Low-cost 低成本声部、Community 共创声部、Codex Auto 自动开发声部、World-model 世界模型声部等长期有效版本。

### 核心概念

- **Version Family / 版本家族**：围绕同一个核心对象建立的版本容器，例如白皮书、残差卡、技能胶囊、共享缓存对象、UI 主题、AgentPolicy、WorldModelSpec、GameSeason 或 CognitiveProsthesis。
- **Voice Track / 声部线**：同一家族中的并行演化轨道。Stable 追求可靠，Experimental 追求创新，Safety 负责阻断风险，Aesthetic 负责体验表达，Low-cost 负责缓存与成本优化，World-model 必须记录不确定性。
- **Version Node / 版本节点**：DAG 中的一个版本节点，记录父节点、子节点、贡献者、变更摘要、内容引用、contentHash、风险等级、隐私等级、验证分、安全分、压缩分、成本画像和安全元数据。
- **Counterpoint Relation / 对位关系**：描述两个版本之间是支持、冲突、扩展、专门化、fork、remix、阻止 public release，还是最终合流。
- **Merge Proposal / 合流提案**：合并不是覆盖旧版本，而是提出一个包含来源版本、冲突摘要、解决计划和风险评估的提案；合流成功后生成新的 VersionNode。
- **Canonical Release / 规范发布版**：某个家族当前可被引用为主旋律的规范版本，但不删除其他仍有效的复调版本。
- **Frozen Archive / 冻结归档**：冻结后的版本不能继续修改，只能作为历史证据、血缘节点或对比参照。
- **Active Polyphony / 当前活跃复调状态**：展示一个版本家族下所有 active、experimental、reviewing、merge_candidate、canonical 等仍可参与讨论与演化的节点。

### 适用对象

复调版本接口覆盖 `whitepaper`、`residual_card`、`wish_card`、`reverse_wish_card`、`skill_capsule`、`shared_cache_object`、`prompt_template`、`review_rubric`、`human_transition_policy`、`ui_theme`、`avatar_morph_template`、`cosmetic_preset`、`agent_policy`、`world_model_spec`、`integration_connector`、`ecosystem_policy`、`game_season`、`cognitive_map_region` 和 `cognitive_prosthesis`。

### 审核、合流与 Canonical

`promoteToCanonical(versionId)` 会执行默认策略：high / critical 风险不能 canonical；quarantined / rejected 不能 canonical；`codex_auto` 声部必须有人类 review；影响 public cache 的版本必须 `privacyLevel = public`；`safetyScore` 必须达到阈值。Safety 声部可以通过 `blocks_public_release` 对位关系阻止其他版本进入公共层。

`proposeMerge(sourceVersionIds, targetVoiceTrackId, resolutionPlan)` 只创建合流提案；`mergeProposalToVersion(proposalId)` 才会生成新的合流节点。原版本不会被覆盖，因此 Stable、安全、低成本、美学和社区版本可以长期共存。

### Codex 并行声部任务

`generateCodexPolyphonyTask(baseTask, voiceTracks)` 会把一个基础任务拆成多个 Codex prompt。每个 prompt 都包含 Goal、Context、Voice Track、Constraints、Done When、Test Command 和 Safety Notes。例如“设计认知大陆地图”可以同时生成 Stable、Aesthetic、Low-cost、Safety、Gameplay 和 World-model 版本。Codex 结果通过 `importCodexTaskResultAsVersion(taskResult)` 进入对应声部，默认处于 `reviewing`，不能绕过人工审核直接成为 canonical。

### World-model 不确定性账本

`world_model` 声部的 VersionNode 必须携带 `uncertaintyMetadata`，包括 assumptions、confidence、knownUnknowns、failureConditions 和 lastValidatedAt。所有预测、模拟、宏观判断都必须承认边界，避免把模型推断伪装成确定事实。

### UI 页面

- `/polyphony`：复调版本总览，展示活跃家族、声部、审核、合流、隔离、Canonical、Experimental 与最近 Codex 分支。
- `/polyphony/families`：版本家族列表。
- `/polyphony/family/[id]`：单个版本家族复调谱面。
- `/polyphony/version/[id]`：版本节点详情、血缘、风险、不确定性账本和审核记录。
- `/polyphony/merge/[id]`：合流提案详情。
- `/polyphony/codex`：Codex 复调任务 prompt 生成页。
- `/polyphony/lineage/[id]`：版本血缘图页面。

## World Experience Gateway / 世界体验接口层

World Experience Gateway 把游戏、单机游戏、网游回放、模拟器、动漫世界、原创虚拟场景和真实体验反馈接入 Cognitive Jelly。它不是外挂系统、不是自动代练系统、不是侵权二创平台；它把游戏世界视为可交互的世界模型沙盒，把动漫世界视为可叙事化世界模型场景，把体验者视为真实反馈传感器。

### 为什么游戏和动漫世界可以作为世界模型沙盒

游戏与模拟器天然包含状态、规则、目标、反馈和失败路径；动漫与原创世界观天然包含叙事、角色动机、情绪和一致性约束。体验者在这些世界中的困惑、失败、沉浸感破坏、策略发现和安全担忧，可以作为 Proof of Experience / 体验证明，回流为残差卡、技能胶囊、世界模型更新和贡献账本事件。

### 允许的游戏接口类型

- 单机游戏 mod / SDK / 存档 / 日志 / 回放分析。
- 官方 API 与授权 SDK。
- 授权赛事数据。
- 用户自愿上传回放。
- 训练场 / 沙盒模式。
...
World Experience Gateway 明确不允许：外挂、绕过反作弊、自动代练、真人网游自动操控、刷分、实时对战作弊建议、未授权数据抓取、破解客户端、IP 复刻和侵权角色复制。`WorldConnector.capabilities.multiplayerLiveControl` 与 `WorldSafetyPolicy.allowLiveMultiplayerControl` 必须恒为 `false`。

### 动漫 / IP 世界观授权边界

允许原创世界观、官方授权 IP 世界、用户自创角色和场景、风格转译但不复刻具体 IP、公开授权素材、非商业同人草案（必须标记不可商用）。不允许未授权复刻动漫/游戏角色、冒充官方授权、复制受保护台词/设定/角色外观、真人未授权数字人格或规避版权检测。

### 什么是体验证明 Proof of Experience

Proof of Experience 证明某个用户在某个世界胶囊中真实观察、体验、判断和反馈过。证明可以来自 manual attestation、replay hash、event log hash、trusted connector 或 human review。证明本身也带隐私等级；未成年人体验数据默认 private / high sensitivity。

### 为什么奖励体验者

体验者奖励不是按在线时长，不按刷次数，也不承诺收益升值。奖励依据是真实反馈质量、证据强度、残差价值、清晰度、有用性和安全信号。高风险领域反馈必须人工审核后再奖励；低质量重复反馈、伪造体验或刷反馈会被 blocked。

### 如何把体验反馈转成残差卡

`routeFeedbackToResidualCard(feedback)` 会把玩法残差、叙事残差、交互残差、情绪残差、世界模型残差和安全残差转成 `ResidualCard`。`mapWorldSceneToResidualCard(worldCapsule)` 可以把世界胶囊本身转译为可验证任务，用于后续技能沉淀和共享缓存治理。

### 真实世界接口安全边界

虚拟世界反馈不能直接等同真实世界结论。真实世界验证必须有外部证据、人类审核或可信数据源；世界模型更新必须保留不确定性账本。涉及金融、医疗、法律、军事、工业安全、未成年人时，系统只能辅助分析，不自动决策。冲突/战争类模拟只允许风险评估、人道救援、冲突降级、灾害避险，不允许作战优化、攻击路径规划、武器部署或目标选择。

### 页面

- `/worlds`：世界体验总览。
- `/worlds/connectors`：世界连接器与授权/只读/反作弊边界。
- `/worlds/capsules`：世界胶囊库。
- `/worlds/session/[id]`：体验会话详情和 Proof of Experience。
- `/worlds/feedback`：体验反馈池。
- `/worlds/rewards`：体验者奖励。
- `/worlds/anime`：动漫/世界观授权边界。
- `/worlds/real-bridge`：真实世界桥与高风险人工审核规则。
